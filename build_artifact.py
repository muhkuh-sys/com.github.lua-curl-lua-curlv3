...
for strMatch in glob.iglob(os.path.join(strCwd, 'lua5.1-curlv3-*.xml')):
    os.remove(strMatch)
...
astrMatch = glob.glob(os.path.join(strCwd, 'lua5.1-curlv3-*.xml'))
if len(astrMatch) != 1:
    raise Exception('No match found for "lua5.1-curlv3-*.xml".')
...
astrCMAKE_COMPILER.append('-Dcurl_DIR=%s' % os.path.join(strCfg_workingFolder, 'lua5.1', 'build_requirements', 'jonchki', 'install', 'dev', 'cmake'))
...
for strMatch in glob.iglob(os.path.join(strCwd, 'lua5.4-curlv3-*.xml')):
    os.remove(strMatch)
...
astrMatch = glob.glob(os.path.join(strCwd, 'lua5.4-curlv3-*.xml'))
if len(astrMatch) != 1:
    raise Exception('No match found for "lua5.4-curlv3-*.xml".')
